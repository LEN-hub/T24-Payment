...
Feature: e-Statement
  ...
  Scenario:process for uerying the account list_SIT pass
  ...
  Scenario:account details page display pass
    Given logon "netSilverEnv_Kevin_SIT" in SIT environment and bypass Vkey
    When account details page display

  @accountService_SIT03
  Scenario:alias setting main process_SIT pass
    Given logon "netSilverEnv_Kevin_SIT" in SIT environment and bypass Vkey
  ...
  Scenario:click Download after the file is generated successfully_SIT pass
    Given logon "netSilverEnv_Kevin_SIT" in SIT environment and bypass Vkey
    When click Download after the file is generated successfully
  @accountService_SIT05
  Scenario:successfully downloaded batch new domestic payee template_SIT pass
    Given logon "netSilverEnv_Kevin_SIT" in SIT environment and bypass Vkey
    When succeeded in downloading the batch added domestic payee template
  @accountService_SIT06
  Scenario:personal information management Basic information management, modify customer nicknames_SIT pass
    Given logon "netSilverEnv_Kevin_SIT" in SIT environment and bypass Vkey
    When modifying customer nicknames
    Then the customer nickname is changed successfully
  @accountService_SIT07
  Scenario:query operation logs of the personal center_SIT pass
    Given logon "netSilverEnv_Kevin_SIT" in SIT environment and bypass Vkey
  ...
  Scenario:personal center online banking transaction query screening conditions_SIT pass
    Given logon "netSilverEnv_Kevin_SIT" in SIT environment and bypass Vkey
    When personal center online banking transaction query screening conditions
  @accountService_SIT09
  Scenario:personal center online banking transaction query submission time selection nearly seven days_SIT pass
    Given logon "netSilverEnv_Kevin_SIT" in SIT environment and bypass Vkey
    When personal center online banking transaction query submission time selection nearly seven days


  @ext123_SIT
  Scenario:Download e-Statement file successfully_SIT pass
...
#    When I see My Downloads List
  ...
  @ext123test122_SIT
  Scenario:Download past e-statements successfully_SIT pass
    Given logon "netSilverEnv_Kevin_SIT" in SIT environment and bypass Vkey
    When I click My Downloads Btn
    When I click View eStatements Btn
#    Then I should direct to the eStatements Overview page
...
#    When I see My Downloads List
  ...
  @ext123test_SIT
  Scenario:Download the electronic notice and successfully generate the download task_SIT pass
    Given logon "netSilverEnv_Kevin_SIT" in SIT environment and bypass Vkey
    When I click My Downloads Btn
...
#    When I see My Downloads List
  ...
  @startUser_SIT
#    启动用户
  Scenario:Start the user operation flow_SIT noPass
    Given logon "netSilverEnv_Kevin_SIT" in SIT environment and bypass Vkey
...
#    用户注销
  Scenario:Process for querying the New users_SIT noPass
    Given logon "netSilverEnv_Kevin_SIT" in SIT environment and bypass Vkey
    When I click on the Enterprise Administration as well user administration
...
#    And fill in and submit the new user information
  ...
  @stopUser_SIT
#    停止用户
  Scenario:Stop the user operation flow_SIT noPass
    Given logon "netSilverEnv_Kevin_SIT" in SIT environment and bypass Vkey
    When I click on the Enterprise Administration as well user administration
  ...
  Scenario:idCard Forgot password users log in normally after resetting their pass
...
#    账户修改
  Scenario:Modify the enterprise management console account_SIT pass
...
#    CA账户SGD开立
  Scenario:Enterprise management console CA account SGD opened_SIT pass
    Given logon "netSilverEnv_Kevin_SIT2" in SIT environment and bypass Vkey
    When I click on the Enterprise Administration Desk and select Account Management
...
#    When I click on the receiving bank drop down box
  ...
  Scenario:Select “>1,000,000 and ≤ 20,000,000” for “Corporate Turnover (SGD)”, “Current Asset Loan” for the loan type, select collateral: real estate, select guarantor: personal guarantee-director; input the loan amount: 200,000; "Overdraft" product loan application successful Pass
    ...
    #    贷款支用。无授权
  @loanApplication4
  Scenario:Accounts Payable Financing Loan Draw Application Process noPass
    Given logon "netSilverEnv_Kevin_SIT" in SIT environment and bypass Vkey
...
#    When I choose to apply for disbursement of financing receivables
    ...
    #    还款。
  @loanApplication7
  Scenario:For accounts receivable financing products, the repayment date is 5 days after the current date, within 1 month, if it does not exceed the due date, and it is fully settled in advance (loan currency: SGD, product term: 30 days), the repayment is successful Pass
    Given logon "netSilverEnv_Kevin_SIT" in SIT environment and bypass Vkey
...
#  单币种 新币+贷款
  ...
  @testcodeNewTrack123
    #正常开户新流程——单币种账户——新币
  Scenario:openAccount_track1 Singapore enterprise_SGD pass
    Given open "netSilverEnv_Kevin_SIT" enterprise net silver page
    ...
    When I enter information in Step 9 and jump to the next step
      ...
      #多币种账户 新币+美元
  Scenario:openAccount Current Account MCA_SGD_USD pass
    Given open "netSilverEnv_Kevin_SIT" enterprise net silver page
    When click open Account page
...
#    Then get Organisation ID
  ...
  @testcodeNew
    #正常开户新流程——单币种账户——新币
  Scenario:openAccountNew Singapore enterprise noPass
    Given open "netSilverEnv_Kevin_SIT" enterprise net silver page
  ...
  Scenario:fixedDeposit_SGD_OneMonth_DoNotRenewOrWithdrawAutomatically_authorization_SIT Pass
    ...
    When Click My Task to find data for authorization
      ...
      #一个月_新币_不转存_全部支取
  @report444
  Scenario:fixedDepositWithdrawFD_SGD_OneMonth_DoNotRenewOrWithdrawAutomatically noPass
    Given logon "netSilverEnv_Kevin_SIT" in SIT environment and bypass Vkey
